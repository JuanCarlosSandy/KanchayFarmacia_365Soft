...
class ReportesVentas extends Controller
{
    ...
    public function descargarReporteGeneralPDF(Request $request)
    {
        ...
        if ($request->filled('fechaInicio') && $request->filled('fechaFin')) {
            ...
            $filtros[] = "Desde: {$request->fechaInicio} Hasta: {$request->fechaFin}";
        }
        ...
        if ($request->filled('ejecutivoCuentas') && $request->ejecutivoCuentas !== 'undefined') {
            $query->where('ventas.idusuario', $request->ejecutivoCuentas);
            $filtros[] = "Ejecutivo: {$request->ejecutivoCuentas}";
        }
        ...
        $pdf->Cell(35, 8, 'N Comprobante', 1, 0, 'C', true);
        $pdf->Cell(35, 8, 'Fecha y Hora', 1, 0, 'C', true);
        $pdf->Cell(50, 8, 'Cliente', 1, 0, 'C', true);
        $pdf->Cell(30, 8, 'Total Venta', 1, 0, 'C', true);
        $pdf->Cell(35, 8, 'Vendedor', 1, 1, 'C', true);

        // Detalles
        $pdf->SetFont('Arial', '', 9);
        $pdf->SetTextColor(0);
        foreach ($ventas as $venta) {
            $pdf->Cell(35, 8, $venta->num_comprobante, 1);
            ...
            $pdf->Cell(35, 8, utf8_decode(substr($venta->vendedor, 0, 25)), 1, 1);
        }
