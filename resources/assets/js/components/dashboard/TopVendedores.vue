  ...
  <Panel>
     <template #header>
        <div class="panel-header">
          ...
          <h4 class="panel-title">TOP VENDEDORES</h4>
        </div>
      </template>
    <DataTable
      v-if="topVendedoresOrdenados.length > 0"
      :value="topVendedoresOrdenados"
      :paginator="true"
      :rows="5"
    ...
    >
      ...
      <Column
        ...
        sortable
        @click="ordenar('cantidadVentas')"
        :headerStyle="{ cursor: 'pointer' }"
      />
      <Column
        field="totalVentas"
        header="Monto de ventas"
        sortable
        @click="ordenar('totalVentas')"
        :headerStyle="{ cursor: 'pointer' }"
        :body="monedaTemplate"
      />
    </DataTable>
...
export default {
  ...
  computed: {
    topVendedoresOrdenados() {
      const copia = [...this.topVendedores];
  ...
  methods: {
    ...
    obtenerTopVendedores() {
      axios
        ...
        .then(res => {
          this.topVendedores = res.data.topVendedores;
        })
        .catch(err => {
          console.error('Error al obtener el top de vendedores:', err);
        });
    },
    nombreTemplate(row) {
    ...
    monedaTemplate(row) {
      const total = row.totalVentas * this.moneda[0];
