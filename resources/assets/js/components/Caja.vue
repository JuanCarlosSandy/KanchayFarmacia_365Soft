  ...
  <main class="main">
    ...
    <Panel>
      ...
      >
        ...
        <Column field="saldoInicial" header="Saldo Inicial" />
        <Column field="ventasContado" header="Ventas al Contado" />
        <Column field="ventasQR" header="Ventas QR" />
        <Column field="saldototalventas" header="Ventas Totales" />
        <Column field="depositos" header="Depósitos Extras" />
        <Column field="salidas" header="Salidas Extras" />
        <Column field="saldoCaja" header="Saldo Caja" />
        <Column field="saldoFaltante" header="Saldo Faltante" />
        <Column field="saldoSobrante" header="Saldo Sobrante" />
        <Column header="Estado">
          <template #body="slotProps">
            ...
            >
              <i
                :class="slotProps.data.estado ? 'pi pi-check' : 'pi pi-times'"
              ></i>
            </Tag>
          </template>
        </Column>
          ...
          <template #body="slotProps">
            <div v-if="slotProps.data.estado">
              <template v-if="slotProps.data.id !== idCajaBotonesSecundarios">
                <Button
                  icon="pi pi-plus"
                  class="p-button-primary p-button-sm p-button-icon-only mr-1"
                  style="padding: 0.3rem 0.4rem; font-size: 0.75rem; width: auto; min-width: unset;"
                  @click="
                    abrirModal2('cajaDepositar', 'depositar', slotProps.data)
                  "
                />
                <Button
                  icon="pi pi-minus"
                  class="p-button-danger p-button-sm p-button-icon-only mr-1"
                  style="padding: 0.3rem 0.4rem; font-size: 0.75rem; width: auto; min-width: unset;"
                  @click="abrirModal3('cajaRetirar', 'retirar', slotProps.data)"
                />
                <Button
                  icon="pi pi-eye"
                  class="p-button-warning p-button-sm p-button-icon-only mr-1"
                  style="padding: 0.3rem 0.4rem; font-size: 0.75rem; width: auto; min-width: unset;"
                  @click="abrirModal4('cajaVer', 'ver', slotProps.data.id)"
                />
                <Button
                  icon="pi pi-calculator"
                  class="p-button-success p-button-sm p-button-icon-only mr-1"
                  style="padding: 0.3rem 0.4rem; font-size: 0.75rem; width: auto; min-width: unset;"
                  @click="
                    abrirModal5('arqueoCaja', 'contar', slotProps.data.id)
                  "
                />
              </template>
              <template v-else>
